...
class Cell:
    ...
    def update1(self, k):
        self.status1[k][0] = STATUS1.content[self.pattern[k][0]][self.pattern[k][1]]
        self.status1[k][1] = STATUS1.content[self.pattern[k][1]][self.pattern[k][0]]
...
class AI:
    ...
    def start(self, width, height):
        ...
        for y in range(4, height + 4):
            for x in range(4, height + 4):
                self.cell[x][y].update1(0)
                self.cell[x][y].update1(1)
                self.cell[x][y].update1(2)
                self.cell[x][y].update1(3)
    ...
    def minmax(self, depth, root, alpha, beta):
                ...
                for y in range(
                    ...
                    for x in range(
                        ...
                        if self.cell[x][y].piece == OXCell.EMPTY and\
                            ...
                            if self.cell[x][y].status4[self.who] == self.A:
        ...
        if nCnd > 1:
            ...
            if nCnd == 0:
                best.value = 0
        maxvalue = cnd[0].value
        for i in range(nCnd):
            if depth == 1 and i > 3:
                continue
            elif depth == 2 and cnd[i].value * 3 + 1 < maxvalue:
                continue
            # elif depth == 3 and cnd[i].value * 4 + 1 < maxvalue:
            #     continue
            ...
            for x in range(self.upperLeftCand.x, self.lowerRightCand.x + 1):
                if self.cell[x][y].piece == OXCell.EMPTY and\
                    ...
                    if x != cnd[0].x or y != cnd[0].y:
                        ...
                        if cnd[nCnd].value > 1:
                            nCnd += 1
        if self.nSt[self.who][self.A] > 0:
            i = 0
            while self.cell[cnd[i].x][cnd[i].y].status4[self.who] != self.A:
                i += 1
            cnd[0] = cnd[i]
            nCnd = 1
            return cnd[:nCnd]

        if self.nSt[self.opp][self.A] > 0:
            i = 0
            while self.cell[cnd[i].x][cnd[i].y].status4[self.opp] != self.A:
                i += 1
            cnd[0] = cnd[i]
            nCnd = 1
            return cnd[:nCnd]

        if self.nSt[self.who][self.B] > 0:
            i = 0
            while self.cell[cnd[i].x][cnd[i].y].status4[self.who] != self.B:
                i += 1
            cnd[0] = cnd[i]
            nCnd = 1
            return cnd[:nCnd]

        if self.nSt[self.opp][self.B] > 0:
            nCnd = 0
            for y in range(self.upperLeftCand.y, self.lowerRightCand.y + 1):
                for x in range(
                    self.upperLeftCand.x, self.lowerRightCand.x + 1
                    ...
                    if self.cell[x][y].piece == OXCell.EMPTY and\
                            (self.cell[x][y].adj1 or self.cell[x][y].adj2):
                        if self.cell[x][y].status4[self.who] >= self.E and self.cell[x][y].status4[self.who] != self.FORBID or\
                                self.cell[x][y].status4[self.opp] >= self.E and self.cell[x][y].status4[self.opp] != self.FORBID:
                            cnd[nCnd].x = x
                            cnd[nCnd].y = y
                            cnd[nCnd].value = self.cell[x][y].prior(self.PRIOR)
                            if cnd[nCnd].value > 0:
    ...
    def whowin(self):
        if self.nSt[self.who][self.A]:
            return self.who
        elif self.nSt[self.opp][self.A]:
            return self.opp
        return -1
    ...
    def quickWinSearch(self):
        ...
        #         logline += (str(self.nSt[m][n]) + " ")
        # print(logline)
        if self.nSt[self.who][self.A] >= 1:
            return 1
        if self.nSt[self.opp][self.A] >= 2:
            return -2
        if self.nSt[self.opp][self.A] == 1:
            for y in range(self.upperLeftCand.y, self.lowerRightCand.y + 1):
                for x in range(
                    self.upperLeftCand.x, self.lowerRightCand.x + 1
                ):
                    if self.cell[x][y].piece == OXCell.EMPTY and\
                            (self.cell[x][y].adj1 or self.cell[x][y].adj2):
                        if self.cell[x][y].status4[self.opp] == self.A:
                            self._move(x, y)
                            q = -self.quickWinSearch()
                            # print(['qws', q])
                            self.undo()
                            if q < 0:
                                ...
                                q += 1
                            return q
        if self.nSt[self.who][self.B] >= 1:
            return 3
        if self.nSt[self.who][self.C] >= 1:
            if self.nSt[self.opp][self.B] == 0 and\
                    self.nSt[self.opp][self.C] == 0 and\
                    self.nSt[self.opp][self.D] == 0 and\
                    self.nSt[self.opp][self.E] == 0:
                return 5
            for y in range(self.upperLeftCand.y, self.lowerRightCand.y + 1):
                for x in range(
                    self.upperLeftCand.x, self.lowerRightCand.x + 1
                    ...
                    if self.cell[x][y].piece == OXCell.EMPTY and\
                            (self.cell[x][y].adj1 or self.cell[x][y].adj2):
                        if self.cell[x][y].status4[self.who] == self.C:
                            self._move(x, y)
                            q = -self.quickWinSearch()
                            self.undo()
                            if q > 0:
                                return q + 1
        if self.nSt[self.who][self.F] >= 1:
            if self.nSt[self.opp][self.B] == 0 and\
                    self.nSt[self.opp][self.C] == 0 and\
                    self.nSt[self.opp][self.D] == 0 and\
                    self.nSt[self.opp][self.E] == 0:
